...
- **改進和優化：**
  1. **此版本優化的功能：**
  2. **日後可能優化的功能：**
     - 設定各別輔助常駐技能的延遲時間，以避免打亂腳本的節奏。

- **Bug 修復：**
     - 放 buff 期間會停止約 10 秒才繼續動作。
     - 解輪失敗時誤觸 "N" 鍵。
     - 在紅黃洞按上沒有點掉視窗。
     - Keyboard_event 隨機秒數前者大於後者。
     - 夢都全圖解輪異常。
     - 角色在左右兩側偵測到輪也不會去解。
     - 運作過程誤點擊滑鼠，開啟視窗導致無法跳舞。
     - 重覆吃消耗類道具，無法自動點掉視窗。
     - 程式開啟Timer即開始計時，如果沒有馬上開始Dance輔助技施放的時間就會重疊。
- **已知問題：**
  1. **已確認：**
...
2. **確保小地圖置於遊戲視窗左上角並顯示地圖名稱（可參考影片），並且技能狀態列（遊戲視窗最上方的那一排）不被遮擋。**
...
- **解地圖輪**
  ...
  2. **為什麼解輪的成功率很低？**
     - 在格蘭蒂斯中，解輪成功率通常可達 90%。如果是在私服中，可能圖片已被調整過。
